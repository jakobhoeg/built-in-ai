...
import {
  ...
  ProgressCallback,
} from "@/util/client-side-chat-transport";
import {
  AIMessage,
  AIMessageAvatar,
  AIMessageContent,
...
import { WebWorkerChatTransport } from "@/util/web-worker-chat-transport";
...
export default function Chat() {
  ...
  const progressCallback: ProgressCallback = useCallback((progress) => {
    ...
    // Hide progress when model is fully loaded
    ...
    <div className="flex flex-col h-[calc(100dvh)] max-w-4xl mx-auto">
      ...
      <AIConversation className="flex-1">
        <AIConversationContent>
          ...
          {/* Loading state - show model progress or thinking */}
          {(status === "submitted" || modelProgress) && (
      ...
      <div className="p-4 bg-background">
        <AIInput onSubmit={handleSubmit}>
          <AIInputTextarea
            ...
            minHeight={48}
            maxHeight={164}
            disabled={!!modelProgress} // Disable input while model is loading
          />
          <AIInputToolbar>
            <AIInputTools>
              <AIInputButton>
            ...
            <AIInputSubmit
              disabled={
                (status === "ready" && !input.trim()) || !!modelProgress
              }
              status={status}
              onClick={
