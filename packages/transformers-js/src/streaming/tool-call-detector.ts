...
export class ToolCallFenceDetector {
  // Support markdown fences, XML-style tags, and Python-style function calls (Qwen, LFM2, IBM Granite)
  ...
  private readonly FENCE_STARTS = this.FENCE_PATTERNS.map((p) => p.start);
  private buffer = "";
  ...
  private findFenceStart(text: string): {
    ...
    for (const pattern of this.FENCE_PATTERNS) {
    ...
    this.PYTHON_STYLE_REGEX.lastIndex = 0;
    const pythonMatch = this.PYTHON_STYLE_REGEX.exec(text);
